...
#define AFIO                ((struct AFIO_TypeDef *) AFIO_BASE)
...
static void hw_can_init(struct CAN_TypeDef* CANx)
{
    ...
    uint32_t GPIO_Remap = GPIO_Remap1_CAN1;
    ...
    if(((__GET_CAN_MCR__(CANx) & CAN_MSR_INAK) == CAN_MSR_INAK)) {
        ...
        __BIT_OR__(__GET_CAN_MCR__(CANx), CAN_MCR_ABOM);
        __BIT_OR__(__GET_CAN_MCR__(CANx), CAN_MCR_AWUM);
        ...
        __BIT_SET__(__GET_CAN_BTR__(CANx), 0x00010008); //正常模式 1000K 75%

//        __BIT_SET__(__GET_CAN_BTR__(CANx), 0x40010008); //回环 1000K 75%
//        __BIT_SET__(__GET_CAN_BTR__(CANx), 0xC0010008); //静默回环 1000K 75%
//        __BIT_SET__(__GET_CAN_BTR__(CANx), 0xC003000C); //静默回环 500K 83.3%
    ...
    __BIT_OR__(__GET_CAN_FA1R__(CANx), 0x00000001);

    __BIT_RESET__(__GET_CAN_FMR__(CANx), FMR_FINIT);
    
    __BIT_SET__(__GET_CAN_IER__(CANx), 0x00000002);
}
...
uint8_t hw_can_receive(struct CAN_TypeDef* CANx, struct hw_can_frame_t* message)
{
    ...
    /* Get the DLC */
    message->DLC = (uint8_t)0x0F & CANx->sFIFOMailBox[FIFONumber].RDTR;
    /* Get the FMI */
    message->FMI = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDTR >> 8);
    /* Get the data field */
    message->DATA[0] = (uint8_t)0xFF & CANx->sFIFOMailBox[FIFONumber].RDLR;
    message->DATA[1] = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDLR >> 8);
    message->DATA[2] = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDLR >> 16);
    message->DATA[3] = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDLR >> 24);
    message->DATA[4] = (uint8_t)0xFF & CANx->sFIFOMailBox[FIFONumber].RDHR;
    message->DATA[5] = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDHR >> 8);
    message->DATA[6] = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDHR >> 16);
    message->DATA[7] = (uint8_t)0xFF & (CANx->sFIFOMailBox[FIFONumber].RDHR >> 24);
    if (FIFONumber == 0x00) {
...
enum hw_platform_errcode_e hw_platform_can_cmd(uint8_t type, WPARAM wParam, LPARAM lParam)
{
    ...
    switch(type) {
        ...
        case HW_CAN_CMD_SEND_CMD:
            ...
            break;
        case HW_CAN_CMD_RECV_CMD:
            hw_can_receive(CAN1, (struct hw_can_frame_t *)wParam);
            break;
        default:
