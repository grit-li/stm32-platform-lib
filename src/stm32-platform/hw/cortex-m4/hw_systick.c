...
static uint32_t hw_systick_get_count_flag(void)
{
    return (__GET_SYSTICK_CTRL__ & SysTick_CTRL_COUNTFLAG);
}

static void hw_systick_select_fclock_source(void)
{
    __BIT_OR__(__GET_SYSTICK_CTRL__, SysTick_CTRL_CLKSOURCE);
}
static void hw_systick_select_stclock_source(void)
{
...
static void hw_systick_counter_enable(void)
{
    __BIT_OR__(__GET_SYSTICK_CTRL__, SysTick_CTRL_ENABLE);
}

static void hw_systick_counter_disable(void)
{
    __BIT_RESET__(__GET_SYSTICK_CTRL__, SysTick_CTRL_ENABLE);
}

static uint32_t hw_systick_get_counter_enable(void)
{
    return (__GET_SYSTICK_CTRL__ & SysTick_CTRL_ENABLE);
}

static void hw_systick_set_reload_value(uint32_t value)
{
    __BIT_COVER_SET__(__GET_SYSTICK_LOAD__, SysTick_LOAD_RELOAD, value);
}

static uint32_t hw_systick_get_reload_value(void)
{
    return (__GET_SYSTICK_LOAD__ & SysTick_LOAD_RELOAD);
}

static void hw_systick_set_current_value(uint32_t value)
{
    __BIT_COVER_SET__(__GET_SYSTICK_VAL__, SysTick_VAL_CURRENT, value);
}

static uint32_t hw_systick_get_current_value(void)
{
    return (__GET_SYSTICK_VAL__ & SysTick_VAL_CURRENT);
}

static uint32_t hw_systick_get_noref_flag(void)
{
    return (__GET_SYSTICK_CALIB__ & SysTick_CALIB_NOREF);
}

static uint32_t hw_systick_get_skew_flag(void)
{
    return (__GET_SYSTICK_CALIB__ & SysTick_CALIB_SKEW);
}

static uint32_t hw_systick_get_calibration_value(void)
{
